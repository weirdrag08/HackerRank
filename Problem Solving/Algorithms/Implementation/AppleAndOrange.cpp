...
void countApplesAndOranges(int s, int t, int a, int b, vector<int> apples, vector<int> oranges) {
    int apples_arr_size = apples .size();
    int oranges_arr_size = oranges.size();
    for(int i = 0 ; i < apples_arr_size ; i++){
        apples[i] += a;
    }
    for(int i = 0 ; i < oranges_arr_size ; i++){
        oranges[i] += b;
    }
    int apple_count = 0;
    int orange_count = 0;
    for(int i = 0 ; i < apples_arr_size ; i++){
        if(apples[i] >= s && apples[i] <= t) apple_count++;
    }
    for(int i = 0 ; i < oranges_arr_size ; i++){
        if(oranges[i] >= s && oranges[i] <= t) orange_count++;
    }
